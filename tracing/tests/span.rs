...
fn handles_to_the_same_span_are_equal() {
    ...
    with_default(subscriber::expect_nothing(), || {
...
fn spans_always_go_to_the_subscriber_that_tagged_them() {
    let subscriber1 = subscriber::expect()
        ...
        .done()
        .run();
    let subscriber2 = subscriber::expect_nothing();
...
fn dotted_field_name() {
    let (subscriber, handle) = subscriber::expect()
        .new_span(span("foo").with_field(field("fields.bar").with_value(&true).only()))
        .done()
        .run_with_handle();
    with_default(subscriber, || {
...
fn move_field_out_of_struct() {
    ...
    let (subscriber, handle) = subscriber::expect()
        ...
        .new_span(span("bar").with_field(field("position").with_value(&debug(&pos)).only()))
        .run_with_handle();
...
fn add_field_after_new_span() {
    let (subscriber, handle) = subscriber::expect()
        .new_span(
            span("foo")
                .with_field(field("bar").with_value(&5)
                .and(field("baz").with_value).only()),
        )
        .record(
            span("foo"),
            field("baz").with_value(&true).only(),
        )
        .enter(span("foo"))
        .exit(span("foo"))
        .drop_span(span("foo"))
    ...
    let (subscriber, handle) = subscriber::expect()
        .new_span(
            span("foo").with_field(
                field("bar")
                    .with_value(&5)
                    .and(field("baz").with_value(&false))
                    .only(),
            ),
        )
        .record(span("foo"), field("baz").with_value(&true).only())
        .enter(span("foo"))
        .exit(span("foo"))
        .drop_span(span("foo"))
        .done()
...
fn record_new_values_for_fields() {
    let (subscriber, handle) = subscriber::expect()
        ...
        .record(span("foo"), field("bar").with_value(&5).only())
        .record(span("foo"), field("baz").with_value(&true).only())
        .enter(span("foo"))
        .exit(span("foo"))
        .drop_span(span("foo"))
...
fn explicit_child_at_levels() {
    let (subscriber, handle) = subscriber::expect()
        ...
        .new_span(span("c").with_explicit_parent(Some("foo")))
        .new_span(span("d").with_explicit_parent(Some("foo")))
        .new_span(span("e").with_explicit_parent(Some("foo")))
        .done()
        .run_with_handle();
...
fn explicit_child_regardless_of_ctx() {
    let (subscriber, handle) = subscriber::expect()
        ...
        .new_span(span("bar"))
        .enter(span("bar"))
        .new_span(span("baz").with_explicit_parent(Some("foo")))
        .exit(span("bar"))
        .done()
        .run_with_handle();
...
fn display_shorthand() {
    let (subscriber, handle) = subscriber::expect()
        .new_span(
            span("my_span")
                .with_field(field("my_field").with_value(&display("hello world")).only()),
        )
        .done()
        .run_with_handle();
    with_default(subscriber, || {
...
fn debug_shorthand() {
    let (subscriber, handle) = subscriber::expect()
        .new_span(
            span("my_span").with_field(field("my_field").with_value(&debug("hello world")).only()),
        )
        .done()
        .run_with_handle();
    with_default(subscriber, || {

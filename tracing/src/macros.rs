...
macro_rules! valueset {
    ...
    (@ { $(,)* $($out:expr),* }, $next:expr, $($k:ident).+ = ~$val:expr, $($rest:tt)*) => {
        $crate::valueset!(
            @ { $($out),*, (&$next, $crate::__macro_support::Option::Some(&$crate::field::valuable(&$val) as &dyn Value)) },
            $next,
            $($rest)*
        )
    };
    ...
    (@ { $(,)* $($out:expr),* }, $next:expr, ~$($k:ident).+, $($rest:tt)*) => {
        $crate::valueset!(
            @ { $($out),*, (&$next, $crate::__macro_support::Option::Some(&$crate::field::valuable(&$($k).+) as &dyn Value)) },
            $next,
            $($rest)*
        )
    };
    ...
    (@ { $(,)* $($out:expr),* }, $next:expr, { $k:expr } = ~$val:expr, $($rest:tt)*) => {
        $crate::valueset!(
            @ { $($out),*, (&$next, Some(&$crate::field::valuable(&$val) as &dyn Value)) },
            $next,
            $($rest)*
        )
    };

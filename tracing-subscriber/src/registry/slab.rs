...
pub struct Span<'a> {
    lock: OwningHandle<RwLockReadGuard<'a, Slab<T>>, RwLockReadGuard<'a, Slot<T>>>,
}
        ...
        N: for<'a> crate::NewVisitor<'a>,
    {
        ...
        loop {
            ...
            if let Some(mut this) = self.inner.try_write() {
                ...
                let slot = Slot<T>::new(span.take().unwrap(), attrs, new_visitor);
...
impl Slab {
    #[inline]
    fn write_slot(&self, idx: usize) -> Option<RwLockWriteGuard<Slot<T>>> {
        self.slab.get(idx).map(RwLock::write)
    }

    #[inline]
    fn read_slot(&self, idx: usize) -> Option<RwLockReadGuard<'_, Slot<T>>> {
        self.slab
            .get(idx)
            .map(RwLock::read)

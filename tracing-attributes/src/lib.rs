        ...
        for stmt in &mut stmts {
            if let Stmt::Item(Item::Fn(fun)) = stmt {
                ...
                if fun.sig.ident == internal_fun.name {
                    *stmt = syn::parse2(gen_body(fun, args, span_declared_name, Some(internal_fun)))
                        .unwrap();
                    break;
                }
            }
            ...
            if let Some(Fields(ref mut fields)) = args.fields {
                for mut field in fields.iter_mut() {
                    if let Some(ref mut e) = field.value {
                        syn::visit_mut::visit_expr_mut(&mut SelfReplacer { ty: async_trait_fun.self_type.clone() }, e);
                    }
                }
            }
        }

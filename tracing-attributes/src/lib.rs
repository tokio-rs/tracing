...
/// ```
///
/// # Logging return values and `Result`s
///
...
/// ```
///
/// ## Transforming the recorded value
///
/// Both `ret` and `err` accept an optional transform argument: a closure or function path that is
/// applied to the value before it is recorded. The transform receives the return value (for `ret`)
/// or error (for `err`) by reference and returns the value to record in its place. This can be used
/// to record a different representation of the value, such as converting an error into a `dyn
/// Error`:
///
/// ```
/// # use tracing_attributes::instrument;
/// # use std::num::ParseIntError;
/// fn as_dyn_error(err: &ParseIntError) -> &(dyn std::error::Error + 'static) {
///     err
/// }
///
/// #[instrument(err(|err| as_dyn_error(err)))]
/// fn my_function() -> Result<u8, ParseIntError> {
///     "not a number".parse()
/// }
/// ```
///
/// To keep function paths unambiguous with future arguments like `Display`, they must have at least
/// two path components and cannot be plain identifiers; to use a function in the current module,
/// prefix it with `self::`:
///
/// ```
/// # mod my_cool_module { // Needed for `self::my_dyn_error` to resolve.
/// # use tracing_attributes::instrument;
/// # use std::num::ParseIntError;
/// # fn as_dyn_error(err: &ParseIntError) -> &(dyn std::error::Error + 'static) {
/// #     err
/// # }
/// #[instrument(err(self::as_dyn_error))]
/// fn my_function() -> Result<u8, ParseIntError> {
///     "not a number".parse()
/// }
/// # }
/// ```
///
/// A transform may be combined with a format mode, in which case the transformed value is recorded
/// using that mode's formatter:
///
/// ```
/// # use tracing_attributes::instrument;
/// #[instrument(ret(Display, |ret| ret + 1))]
/// fn my_function() -> i32 {
///     42
/// }
/// ```
///
/// When a transform is given without an explicit format mode, the transformed value is recorded
/// directly via its [`Value`] implementation (rather than through its `Debug` or `Display`
/// formatter). This preserves the value's type, so consumers can downcast it -- for example,
/// recording a `&dyn std::error::Error` as in the first example above.
///
/// A function path (`err(some::func)`) is preferable to a closure when the transform returns a
/// reference borrowed from its argument. A `fn`'s lifetime elision ties the output lifetime to the
/// input, whereas an equivalent closure infers two independent lifetimes and fails to compile.
///
/// [`Value`]: https://docs.rs/tracing/latest/tracing/trait.Value.html
///
/// If a `target` is specified, both the `ret` and `err` arguments will emit outputs to the declared
/// target (or the default channel if `target` is not specified).
///
/// The `ret` and `err` arguments can be combined in order to record an event if a function returns

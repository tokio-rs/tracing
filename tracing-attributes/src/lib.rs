...
fn gen_block(
    ...
    let span = (|| {
        ...
        let param_names: Vec<(Ident, (Ident, RecordType))> = params
            ...
            .filter_map(|(x, record_type)| {
                ...
                if self_type.is_some() && x == "_self" {
                    Some((Ident::new("self", x.span()), (x, record_type)))
                } else if x.to_string().starts_with('_') {
                    // Ignore unused parameters (Aka, the parameter starts with underscore, e.g. _arg: usize).
                    None
                } else {
                    Some((x.clone(), (x, record_type)))
                }
            })
            .collect();
        ...
        if let Some(skip) = args
            .skips
            .iter()
            .find(|skip| skip.to_string().starts_with('_'))
        {
            return quote_spanned! {skip.span()=>
                compile_error!("unnecessary skipping: the unused parameter is auto skipped by default")
            };
        }
